...
const StaffPerformance = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          setIsLoading(true);
          try {
            const response = await AxiosInstance.get("/orders/staff-performance");
  ...
  return (
    ...
    <Card>
      <Card.Body>
        <Card.Title className="h4 mb-4">Staff Performance</Card.Title>
            ...
            {products?.map((item, index) => (
              ...
              >
                <Card className="h-100" style={{ borderBottom: '4px solid rgb(0, 60, 255)' }}>
                  <Card.Body>
                    <Card.Title className="h5 mb-4">{item?._id}</Card.Title>
                    
                    <Row className="mb-2">
                      <Col xs={6} className="text-muted">Total Sales:</Col>
                      <Col xs={6} className="text-end fw-bold">{(item?.totalSales).toFixed(2)}</Col>
                    </Row>
                    
                    <Row className="mb-2">
                      <Col xs={6} className="text-muted">Total Order Count:</Col>
